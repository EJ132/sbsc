...
const config = {
  ...
  callbacks: {
    ...
    cardNonceResponseReceived: function (errors, nonce, cardData) {
      ...
      ShopHelper.squareSetShippingCost(
        localStorage.getItem("checkout-id")
      ).then((res) => {
        localStorage.setItem(
          "checkout-version",
          //   (parseInt(localStorage.getItem("checkout-version")) + 1).toString()
          res.order.version.toString()
        );

        ShopHelper.setShipping(
          localStorage.getItem("shipping-id"),
          localStorage.getItem("rate-id")
        );
